...
//增加agora的js
document.write("<script language='javascript' src='/static/js/Agora/agora-rtm-sdk-1.0.0.js'></script>");
...
$(document).ready( function() {
    ...
    //动态AJAX修改CPU
    getPercentageData();
    window.setInterval(loadnumber,1000);
    // updateSettings();
    ...
    $.ajax({
        ...
        success:function(data){
            $.each(data.data, function(i, item) {
                var f_balan = $('#f_balance');
                var s_clicks = $('#s_clicks');
                var t_subscribe = $('#t_subscribe');
                var f_support = $('#f_support');
                f_balan.text(data.data[iCount].temperature);
                s_clicks.text(data.data[iCount].dampness);
                t_subscribe.text(data.data[iCount].PH);
                f_support.text(data.data[iCount].DB);
            })
        }
    });
    iCount++;
...
function getPercentageData() {
    ...
    var restored = document.getElementById('restored');
    var recovering = document.getElementById('recovering');
...
var handleProfileSkillPie = function () {
    ...
    chart1 = window.chart = $('#CPU').data('easyPieChart');
}
...
function loadnumber(){
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            $.each(data.keyValue, function(i, item) {

                // 赋给环图值
                num = data.keyValue[count].value;

                // 五个percent的显示值
                CPU.children[0].innerHTML = num;
                restored.innerHTML = data.keyValue[count]['value'];
                recovering.innerHTML = data.keyValue[count + 1]['value'];
                dll.innerHTML = data.keyValue[count + 2]['value'];
                req.innerHTML = data.keyValue[count + 3]['value'];

                //五个percent的进度线
                $('#CPU').attr('data-percent',num);

                $('#restoredPercent').css('width',restored.innerHTML + '%');
                checkValue(restored.innerHTML,$('#restoredPercent'));

                $('#recoveringPercent').css('width',recovering.innerHTML + '%');
                checkValue(recovering.innerHTML,$('#recoveringPercent'));

                $('#dllPercent').css('width',dll.innerHTML + '%');
                checkValue(dll.innerHTML,$('#dllPercent'));

                $('#reqPercent').css('width',req.innerHTML + '%' );
                checkValue(req.innerHTML,$('#reqPercent'));

                //这里重画easypiechart
                chart1.update(Math.round(num));
...
function setup_chatbox_demo() {
    ...
    $('#send-msg-js').click(function() {
        ...
        if (msg_input.length) {
            var msg_input = $.istying.val();
            id++;
            $('.tab-pane.active > div > .chat-messages').append('<p id="message-dynamic-' + id + '" class="message-box you"><img src="/static/img/avatar/avatar_1.jpg" alt=""><span class="message"><strong>Me</strong><span class="message-time">By Victoria at'+ date + '</span><span class="message-text">' + msg_input + '</span></span></p>')
            $('.tab-pane.active > div > .chat-messages #message-dynamic-' + id).hide().fadeIn(750);
            console.log(msg_input.trim() + id);
            //agora发送
            client.sendMessageToPeer({text: msg_input}, '1234',)
                .then(sendResult => {
                    if (sendResult.hasPeerReceived) {
                        /* 远端用户收到消息的处理逻辑 */
                    } else {
        ...
        //JLing机器人代码
         client.on('MessageFromPeer', ({ text }, peerId) => {
            ...
            html = '<p id="message-dynamic-' + id + '" class="message-box"><img src="/static/img/avatar/avatar_0.jpg" alt=""><span class="message"><strong>JLing管家</strong><span class="message-time">By JLing管家  at '+ date + '</span><span class="message-text">' + temp + '</span></span></p>'
                $('.tab-pane.active > div > .chat-messages').append(html);
                $('.tab-pane.active > div > .chat-messages #message-dynamic-' + id).fadeIn(1550);
                $('.chat-messages').scrollTop($('.chat-messages')[0].scrollHeight);
                $.istying.val('');
             //判断是否为开关数据
             if(text.indexOf("{")!=-1){
                 ...
                 var obj=eval(json);
                 var dating1=document.getElementById("dating1");
                 dating1.value=obj.D1;
                 var dating2=document.getElementById("dating2");
                 dating2.value=obj.D2;
                 var dating3=document.getElementById("dating3");
                 dating3.value=obj.D3;
                 var keting1=document.getElementById("keting1");
                 keting1.value=obj.K1;
                 var keting2=document.getElementById("keting2");
                 keting2.value=obj.K2;
                 var keting3=document.getElementById("keting3");
                 keting3.value=obj.K3;
                 var woshi1=document.getElementById("woshi1");
                 woshi1.value=obj.W1;
                 var woshi2=document.getElementById("woshi2");
                 woshi2.value=obj.W2;
                 var woshi3=document.getElementById("woshi3");
                 woshi3.value=obj.W3;
                 var shufang1=document.getElementById("shufang1");
                 shufang1.value=obj.S1;
                 var shufang2=document.getElementById("shufang2");
                 shufang2.value=obj.S2;
                 var shufang3=document.getElementById("shufang3");
                 shufang3.value=obj.S3;
    ...
    $.istying.on('keyup', function(e) {
        ...
        if (e.keyCode == 13) {
            ...
            if (msg_input.length) {
                // var msg_input = $.istying.val();
                // id++;
                // $('.tab-pane.active > div > .chat-messages').append('<p id="message-dynamic-' + id + '" class="message-box you"><img src="img/avatar/avatar_0.jpg" alt=""><span class="message"><strong>Me</strong><span class="message-time">by Victoria at 14:25pm, 4th Jan 2013</span><span class="message-text">' + msg_input + '</span></span></p>')
                // $('.tab-pane.active > div > .chat-messages #message-dynamic-' + id).hide().fadeIn(750);
                // console.log(msg_input.trim() + id);
                //                $('.chat-messages').scrollTop($('.chat-messages')[0].scrollHeight);
                // $.istying.val('');
